...
@section('content')
    <div id="admin-content">
        <div class="container">
            <h2 id="admin-content-title">Edit Article</h2>
            @if ($errors)
            ...
            <div class="jumbotron col-sm-12">
                <form role='form' method="post"  data-id="{{ $data['id'] }}" enctype="multipart/form-data" class="col-sm-12 main-form" action="{{ route('postAdminArticlesEdit',['id'=>$data['id']]) }}">
                    {{ csrf_field() }}
                    ...
                    <div class="form-group col-sm-8">
                        ...
                        <select class="form-control  js-example-placeholder-single" name="type_id" style="color: #000 !important; border: 1px solid #ccc !important;">
                            @foreach($all_articletypes as $item)
                                <option value="{{ $item->id }}" <?=$data['type_id']== $item->id ? "selected" : null?> >{{ $translate::text($item->title,env('LANG_FOR_ADMIN_RU')) }}</option>
                            @endforeach
                        </select>
                    </div>
...
<script>
    $(document).ready(function () {
        ...
        <?php foreach($images as $item){
            ...
            $('span.set-main[data-key="<?php echo $item->id?>"]').addClass('visitable-hide');
